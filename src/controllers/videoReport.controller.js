    ...
    return res
      ...
      .json(
        new ApiResponse(
          201,
          videoReportResponse,
          "Video report created successfully"
        )
      );
  } catch (error) {
    return res
...
// Get all reports on basis of user
const GetAllReportsByUserId = asyncHandler(async (req, res) => {
  try {
    const loggedInUser = req.user._id;
    const getAllReport =
      await videoReportService.GetAllReportsByUserId(loggedInUser);
    return res
      .status(201)
      .json(
        new ApiResponse(201, getAllReport, "All Reports fetched successfully")
      );
  } catch (error) {
    return res
      .status(500)
